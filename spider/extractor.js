...
/**
 * According drill_rules extracting all links from html string
 ...
 * @param link
 * @returns [alias name,alias]   tets的第十几个步骤
 */
...
extractor.prototype.arrange_link = function(links){
    ...
    for(var i=0;i<links.length;i++){
        ...
        if(matched_driller.length>0){
            ...
            if(linkobj[driller_lib]==undefined){
                linkobj[driller_lib]=[];
            }
            if(driller_rule['id_parameter'] && driller_rule['id_parameter'].length > 0){
                var id_parameter = driller_rule['id_parameter']; /*该网址可以带的有效参数，如果数组第一个值为#，表示过滤一切参数*/
                var urlobj = url.parse(link);
