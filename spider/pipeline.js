...
var jobsAPI = require('../lib/mymongodb/jobmodel.js');
var MongodbAPI = require('../lib/mymongodb/my-mongodb.js');
...
// var poolModule = require('generic-pool');   //generic-pool模块是nodejs的一个第三方模块,其作用为提供一个通用的连接池模块,可以通过generic-pool实现对tcp连接池或者mysql数据库连接池等的管理。
...
pipeline.prototype.assembly = function(callback){
    ...
    if(this.spiderCore.settings['save_content_to_mongodb']===true){
...
pipeline.prototype.save_links = function(page_url,version,linkobjs,drill_relation,callback){
    ...
    async.whilst(
        ...
        function (cb) {
            ...
            if(typeof(t_driller_rules)!='object'){
                t_driller_rules = JSON.parse(t_driller_rules);
            }
            ////////////////////////////////link array/////////////////////////
            var sindex = 0;
            async.whilst(
                function () { 
                    return sindex < links.length; 
                },
                function (scb) {
                    ...
                    async.waterfall([
                        ...
                        function(mcb){
                            ...
                            if(t_driller_rules['id_parameter']){
                                ...
                                if(typeof(id_parameter)!='object'){
                                    id_parameter = JSON.parse(id_parameter);
                                }
                        ...
                        function(final_link, mcb){
                            if(final_link != link){
                                logger.debug('Transform: ' + link + ' -> '+final_link);
                            }
...
pipeline.prototype.save_content = function(pageurl,content,extracted_data,js_result,referer,urllib,drill_relation,callback){
    if(this.hbase_via_http)
        return this.save_content_vhttp(pageurl,content,extracted_data,js_result,referer,urllib,drill_relation,callback);
...
pipeline.prototype.save_binary = function(pageurl,content,referer,urllib,drill_relation,callback){
    if(this.hbase_via_http){
        return this.save_binary_vhttp(pageurl,content,referer,urllib,drill_relation,callback);
    }
 ...
 * @param extracted_info    //test里面的最后一个步骤啊
 */
pipeline.prototype.save =function(extracted_info,callback){
    var pipeline = this;

    if(this.spiderCore.settings['test']){   //test单个页面的情况
        var fs = require('fs');
        ...
        async.series([
            ...
            function(cb){
                ...
                if(extracted_info['extracted_data']){
                    var newJobinfo = new jobsAPI.Jobinfo({
                        url: extracted_info['url'],
                        mainimg: extracted_info['extracted_data']['mainimg'],
                        addname: extracted_info['extracted_data']['addname'],
                        tags: extracted_info['extracted_data']['tags'],
                        abstracts: extracted_info['extracted_data']['abstracts'],
                        imgs: extracted_info['extracted_data']['imgs']
                    });

                    jobsAPI.create(newJobinfo,function (err) {
                        if (err) {
                            console.log('fail');
                        }else{
                            console.log('suc');
                    ...
                    var fs = require('fs');
                    var path = require('path');
                    var htmlfile = path.join(__dirname,'..', 'instance','zy1','results',fileName1);
                    var resultfile = path.join(__dirname,'..', 'instance','zy1','results',fileName2);
                    fs.writeFile(htmlfile,extracted_info['content'],'utf8', function (err) {
                        if (err){
                            throw err;
                        }
                        logger.debug('Content saved, '+htmlfile);
                        delete extracted_info['content'];
                        fs.appendFile(resultfile,JSON.stringify(extracted_info['extracted_data']),'utf8',function(err){
                            if (err){
                                throw err;
                            }
                            logger.debug('Crawling result saved, '+resultfile);
            ...
            function(cb){
                if('pipeline' in pipeline.spiderCore.spider_extend){
                    pipeline.spiderCore.spider_extend.pipeline(extracted_info,cb);//spider extend
                }else {
                    cb(null);
                }
