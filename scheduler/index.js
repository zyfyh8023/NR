...
var scheduler = function(settings){
    ...
    this.priotities_updated = 0;  //规则的更新最新时间
 ...
 * 步骤2
 * assembly: 连接数据库   初始化
 * @读取driller_info_redis_db数据
 * @初始化操作url_info_redis_db数据edit
 */
scheduler.prototype.assembly = function(){
    var self = this;
    var dbtype = 'redis';
    if(self.settings['use_ssdb']){
        dbtype = 'ssdb';
    }
    async.series([
        function(cb){
        ...
        self.refreshPriotities();    //第一个步骤
    });
}
/**
 * 步骤3
 * refresh driller rules periodically     第一个步骤
 */
scheduler.prototype.refreshPriotities  = function(){
    ...
    var redis_cli = this.redis_cli0;
        redis_cli.get('updated:driller:rule',function(err,value){
            ...
            if(this.priotities_updated !== parseInt(value)){ //规则更新的最新时间
                    ...
                    for(var i=0; i<values.length; i++){
                        (function(key,scheduler){
                            redis_cli.hgetall(key, function(err,value){
                                ...
                                if(scheduler.tmp_driller_rules == undefined){
                                    scheduler.tmp_driller_rules = {};
                                }
                                var isActive = value['active']=='true'||value['active']==true||value['active']=='1'||value['active']==1?true:false;
                                if(isActive){
                                    logger.debug('Load rule: '+key);
                                    if(scheduler.tmp_driller_rules[value['domain']] == undefined){
                                        scheduler.tmp_driller_rules[value['domain']]={};
                                    }
                                    ...
                                    scheduler.tmp_priority_list.push({
                                        'key':key,
                                        'rate':rate,
                                        'rule':value['schedule_rule'],  /*调度方式，FIFO或者LIFO*/
                                        'interval':parseInt(value['schedule_interval']), /*重新调度的周期，单位秒*/
    ...
    redis_cli.llen('queue:scheduled:all',function(err, queue_length) {
        ...
        async.whilst(
            ...
            function(cb) {  
                ...
                // if(index==1){
                //     console.log(xdriller);
                //     console.log(avg_rate);
                //     console.log(left);
                //     process.exit();
                // }
                //--check reschedule------------- 一天更新一次reSchedule？？？？？
                if((new Date()).getTime() - xdriller['first_schedule'] >= xdriller['interval']*1000){
                //if((new Date()).getTime() - xdriller['first_schedule'] >= 0){
...
scheduler.prototype.reSchedule = function(driller,index){
    ...
    //规则配置里面的seed有的情况 periodically append seeds to queue
    var links = [];
    ...
    // if(index==2){
    //     console.log(scheduler.schedule_version);
    //     process.exit();
    // }
            ...
            scheduler.updateLinkState(link,'schedule',scheduler.schedule_version,function(bol){
                if(bol){
                    ...
                    scheduler.redis_cli0.rpush('queue:scheduled:all',link,function(err, value){
                        logger.info('reschedule url: '+link+" 成功啦！");
                    });
                }else{
    ...
    // console.log(this.schedule_version);
    // console.log(this.priotity_list[index]['first_schedule']);
    // console.log(driller['key']);
    // process.exit();

    // update first schedule time
    ...
    this.redis_cli0.hset(driller['key'],'first_schedule',this.schedule_version,function(err,value){
        if(err){
            logger.error('update first schedule time for '+driller['key']+' failure');
        }else {
            logger.debug('update first schedule time for '+driller['key']+' successful');
        }
    });
}
/**
...
scheduler.prototype.doScheduleExt = function(xdriller,avg_rate,more,cb){
    ...
    redis_cli0.llen('urllib:'+xdriller['key'],function(err, queue_length) {
        ...
        // console.log('urllib:'+xdriller['key']);
        // console.log(queue_length);
        // console.log(ct);
        // console.log(act);
        // process.exit();
...
/**
 * detect link which driller rule matched（寻找符合规范的link）
 * @param link
 * @returns {string}
 */
...
scheduler.prototype.transformLink = function(link,urllib){
    ...
    if(this.driller_rules[domain]&&this.driller_rules[domain][drill_alias]){
        ...
        if(typeof(driller_rule)!='object'){
            driller_rule = JSON.parse(driller_rule);
        }
            ...
            if(Array.isArray(id_parameter)&&id_parameter.length>0){
                ...
                for(var x=0;x<id_parameter.length;x++){
                    ...
                    if(parameters.hasOwnProperty(param_name)){
                        new_parameters[param_name] = parameters[param_name];
                    }
                }
...
scheduler.prototype.checkURL = function(url,interval,callback){
    ...
    redis_cli1.hgetall(kk,function(err,values){
        ...
        if(!values || isEmpty(values)){
            logger.error(url+' not exists in urlinfo');return callback(false);
        }
...
scheduler.prototype.updateLinkState = function(link,state,version,callback){
    ...
    this.redis_cli1.hgetall(urlhash,function(err,link_info){
        ...
        }else{   //在seed里面的地址，要先判断是否符合规则里面配置的href规范，符合在加入数据库
            var trace = scheduler.detectLink(link); //返回值  'driller:'+domain+':'+alia
            if(trace!=''){
                trace = 'urllib:' + trace;
                var urlinfo = {
                    'url':link,
                    'trace':trace,    //'urllib:driller:'+domain+':'+alia
                    'referer':'',
                ...
                if(version){
                    urlinfo['version'] = version;//update version
                }
